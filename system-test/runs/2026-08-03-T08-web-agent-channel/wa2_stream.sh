...
echo "[wa2] стрим пошёл: $(date -u +%H:%M:%S)"
curl -sS -N --max-time 300 -X POST "$GW/aichat-vbai/stream" \
  -H "Authorization: Bearer $TOKEN" -H "Content-Type: application/json" \
  -d @$R/wa2_payload.json > $R/wa2_stream.log 2>&1
echo "[wa2] стрим закрыт: $(date -u +%H:%M:%S), байт: $(wc -c < $R/wa2_stream.log)"
