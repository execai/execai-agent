...
🌐 **Сайт:** [execai.ru](https://execai.ru) · 💬 Веб-чат: [chat.execai.ru](https://chat.execai.ru) · 🖥 Редактор: [ExecAI Studio](https://github.com/execai/execai-studio)

---

# execai — терминальный AI-агент

CLI-агент в духе Claude Code, который реально работает на твоей машине: читает файлы, запускает команды, ходит в kubernetes, отвечает в чате. Поддерживает **11 источников** — наш ExecAI + твои подписки Z.ai / Kimi Code / Anthropic / OpenAI + локальные Claude Code / OpenAI Codex CLI / Ollama (cloud или local). Между ними переключается на лету общей историей.
...
| **openrouter** | OpenRouter — все вендоры одним ключом (Claude, GPT, Gemini, DeepSeek, Qwen…) | Pay-per-token OpenRouter |
...
| **openrouter** | 400+ моделей всех вендоров, список тянется из их каталога |
...
### Поиск по памяти — сначала своя, потом чужая

```
› /memory find тарифы
```

Ищет в собственной памяти агента. Если там пусто — смотрит память других
агентов на этой машине: каталог проекта у Claude Code
(`~/.claude/projects/<проект>/memory/`), его глобальную папку и `CLAUDE.md` — и
**предлагает забрать находки к себе**. Молча ничего не копируется: найти и
присвоить — разные вещи, и второе решаешь ты.

`/memory import` заберёт то, что другие инструменты оставили рядом с проектом
(`CLAUDE.md`, `.cursorrules`, `AGENTS.md`…), `/memory export` выгрузит нашу
память в markdown.
...
Привязка делает два дела: запоминает «этот каталог на этой машине = тот проект» и добавляет в проект обычную запись инструмента — как ssh-профиль, — поэтому машина появляется в карточке проекта с тумблером вкл/выкл. В интерфейсе машина подписывается «алиас (короткий id)», полный id — по наведению; стабильный id переживает повторный login.
...
- **Всё вне списка агент спрашивает прямо в веб-чате** *(с v6.33)*: во время задачи появляется вопрос с той командой, которую агент собирается выполнить, и теми же вариантами, что в TUI — «Разово», «Весь инструмент в этой задаче», «Эту команду в этой задаче», «НАВСЕГДА», «Отклонить». «НАВСЕГДА» записывается в `permissions.json` на машине. Не ответил за ~2 минуты (или вкладка закрыта) — агент получает отказ: молчание никогда не расширяет права.
