...
- **Что разрешено — твой `permissions.json`** (то, что ты отмечал «Всегда» в TUI). Всё вне списка отклоняется — рядом нет человека, который мог бы подтвердить. Пустой файл → разрешено всё, с громким предупреждением при старте.
...
В этом режиме выключен AskUser (некому отвечать), а предел итераций ниже (30 на задачу).
