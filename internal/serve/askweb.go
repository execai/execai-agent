...
func (a *webApprover) AskApprove(toolName string, args json.RawMessage, summary string) agent.ApproveDecision {
	...
	case answerExact:
		...
		return agent.ApproveOnce
	case answerAlways:
		// Записываем на машине: иначе следующий запуск спросит снова, и
		// кнопка «Всегда» окажется обманом.
		if a.perms != nil {
			a.perms.AddTool(toolName)
			if err := a.perms.Save(); err != nil {
				fmt.Fprintf(os.Stderr, "не удалось сохранить разрешение: %v\n", err)
			}
		}
		a.audit.record("allow", toolName, summary, "ответ из чата: всегда (записано в permissions.json)")
		return agent.ApproveOnce
	default:
