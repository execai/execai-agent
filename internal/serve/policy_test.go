...
func TestWebApprover_AlwaysPersists(t *testing.T) {
	...
	// Контракт изменился 15.08.2026: согласователь ПЕРЕДАЁТ решение, а
	// записывает его цикл.
	//
	// Раньше веб-согласователь сам звал AddTool(name) и возвращал ApproveOnce.
	// С появлением масштаба разрешений (каталог, домен, отдельный секретный
	// файл) это стало опасно: ответ «навсегда» на ОДИН каталог записывался как
	// «весь Read» и открывал чтение всей машины. Масштаб знает только цикл.
	if d := a.AskApprove("Bash", json.RawMessage(`{"command":"go test"}`), "go test"); d != agent.ApproveAlways {
		t.Fatalf("решение %v — «всегда» должно доехать до цикла, а не записываться здесь", d)
	}
	...
	if reloaded.HasTool("Bash") {
		t.Error("согласователь записал разрешение сам — цикл не сможет сузить его до каталога или домена")
	}
}
