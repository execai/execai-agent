...
type In struct {
	...
	// command: name = state | set_model | set_source, value — аргумент
...
func Run(ctx context.Context, cfg *config.Config, opts Options) error {
	...
	sourceModels := func() []NamedItem {
		...
		// Живой список с /connect — приоритет; статические каталоги — фолбэк.
		if len(active.AvailableModels) > 0 {
			items := make([]NamedItem, 0, len(active.AvailableModels))
			for _, id := range active.AvailableModels {
				items = append(items, NamedItem{ID: id})
			}
			return items
		}
		var ms []llm.Model
	...
	stateOut := func() Out {
		...
		for _, sub := range subs.List() {
			o.Sources = append(o.Sources, NamedItem{ID: sub.Provider,
	...
	for in := range userQ {
		...
		case "command":
			...
			case "load_chat":
				...
				// Упрощённый реплей для UI — как replayHistoryToUI в TUI.
...
func (a *ideApprover) AskApprove(toolName string, args json.RawMessage, summary string) agent.ApproveDecision {
	...
	a.s.emit(Out{
		...
		Options: []AskOption{
			...
			{answerAlways, "НАВСЕГДА", "весь " + toolName + " без вопросов — запишется в permissions.json"},
	...
	case answerAlways:
		if a.perms != nil {
			a.perms.AddTool(toolName)
			if err := a.perms.Save(); err != nil {
