...
func (m *tuiModel) applySubscriptionSource() {
	...
	case subscriptions.SourceZAIAPI:
		// Same GLM models, but reached through the open platform. Prefer the
		// real list fetched at /connect; GLMModels() is the network fallback.
		if len(active.AvailableModels) > 0 {
			m.models = buildZAIAPIDynamicCatalog(active.AvailableModels)
		} else {
			m.models = llm.ZAIAPIModels()
		}
...
func (m *tuiModel) startConnectFlow(arg string) string {
	...
	if provider != "zai" && provider != "zai-api" && provider != "anthropic" && provider != "kimi" && provider != "kimi-api" && provider != "openai" {
	...
	if provider == "anthropic" || provider == "kimi-api" || provider == "zai-api" || provider == "openai" {
		plan = "api"
	}
	if provider == "zai-api" && baseURL == "" {
	...
	// Z.ai open platform: the same OpenAI-compatible /models with validation.
	// A Coding Plan key pasted here returns 401/403 — reject it and say where
	// such a key belongs, otherwise the user gets a confusing failure later.
	if provider == "zai-api" {
		avail, status := fetchKimiAvailableModelsWithStatus(baseURL, apiKey)
		// 401/403 — not a key for this endpoint. 402/429 — the key is valid but
		// has no pay-per-token balance, which is exactly what a Coding Plan key
		// looks like here: the subscription is billed separately, so the open
		// platform sees a zero balance. Both mean "wrong key", and catching it
		// now beats a confusing failure on the user's first message.
	...
	if provider == "kimi" || provider == "kimi-api" || provider == "zai-api" {
...
// buildZAIAPIDynamicCatalog — catalog from the real /models list of the Z.ai
// open platform. Model IDs match the Coding Plan ones (glm-5.2, glm-4.7…), but
// Provider is "zai-api": the two catalogs must not be mixed up, otherwise a
// request goes to the wrong endpoint with the wrong key.
func buildZAIAPIDynamicCatalog(ids []string) []llm.Model {
	// Iterate the priority list, not the server's order — otherwise glm-4.7
	// could become primary just because it came first in the response.
	primaryOrder := []string{"glm-5.2", "glm-5.1", "glm-5", "glm-4.7", "glm-4.6"}
	idsSet := map[string]bool{}
	for _, id := range ids {
		idsSet[id] = true
	}
	...
	models := make([]llm.Model, 0, len(ids))
	for _, id := range ids {
		mod := llm.Model{ID: id, Provider: "zai-api", HasTools: true, Tier: "standard", Name: id}
		if strings.HasPrefix(id, "glm-5") {
			mod.Tier = "flagship"
		}
		if id == primaryID {
			mod.IsPrimary = true
