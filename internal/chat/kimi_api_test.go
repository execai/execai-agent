...
// TestApplySubscriptionSource_KimiAPI — /source kimi-api подтягивает MoonshotModels,
// primary модель = kimi-latest.
func TestApplySubscriptionSource_KimiAPI(t *testing.T) {
	...
	// Должны быть моонсоновские (kimi-api provider'а).
...
// TestMoonshotModels_ContainsExpected — sanity: MoonshotModels() отдаёт что ждём.
...
// TestSourceLabel_KimiAPI — статус-бар для kimi-api.
