...
var allCommands = []suggestItem{
	...
	{insert: "/security ", label: "/security", hint: "hint.security"},
	...
	{insert: "/connect openrouter ", label: "/connect openrouter", hint: "hint.connect.openrouter"},
	...
	{insert: "/memory find ", label: "/memory find", hint: "hint.memoryFind"},
...
func (m *tuiModel) handleCommand(line string) (tea.Model, tea.Cmd) {
	...
	case cmd == "/login":
		...
		m.refreshViewport()
		return m, func() tea.Msg { return autoStartLoginMsg{} }
	case cmd == "/security" || strings.HasPrefix(cmd, "/security "):
		arg := strings.TrimSpace(strings.TrimPrefix(cmd, "/security"))
		var b strings.Builder
		if arg == "" {
			// Без аргумента — показываем текущий уровень и что он означает.
			// Настройка безопасности, смысл которой надо угадывать, не работает.
			b.WriteString("\n" + i18n.T("security.title") + "\n")
			for _, l := range security.Levels() {
				mark := "  "
				if l == security.Current() {
					mark = "▸ "
				}
				b.WriteString(fmt.Sprintf("%s%-9s — %s\n", mark, l.String(), i18n.T("security.desc."+l.String())))
			}
			b.WriteString("\n" + i18n.T("security.howto") + "\n")
		} else {
			lvl := security.Parse(arg)
			if lvl.String() != strings.ToLower(arg) {
				b.WriteString("\n" + i18n.Tf("security.unknown", arg) + "\n")
			} else {
				security.Set(lvl)
				m.cfg.SecurityLevel = lvl.String()
				if err := config.Save(m.cfg); err != nil {
					b.WriteString("\n" + err.Error() + "\n")
				} else {
					b.WriteString("\n" + i18n.Tf("security.set", lvl.String(), i18n.T("security.desc."+lvl.String())) + "\n")
				}
			}
		}
		m.uiSegments = append(m.uiSegments, segment{kind: "system_hint", body: b.String()})
