...
// TestBuildOpenAIDynamicCatalog — primary picker берёт лучшую из доступных.
func TestBuildOpenAIDynamicCatalog(t *testing.T) {
	cases := []struct {
		...
		{"unknown only", []string{"whisper-1", "text-embedding-3"}, "whisper-1"}, // первый по списку
	}
...
// TestOpenAIModels_ContainsExpected — hardcoded fallback каталог.
