...
* Some reads and all network calls need the user's permission, which you cannot
  request. If a tool is refused, do not retry and do not work around it: state
  plainly in your result what you could not access and why, so the main agent
  can ask the user.
...
// subagentApprover отказывает во всём, о чём его спросили.
//
// Спрашивают его теперь по делу: чтение вне проекта и сетевые запросы
// проходят периметр и у субагента. Разрешения, которые человек выдал раньше,
// проверяются ДО согласователя (agent.New грузит permissions), поэтому
// «разрешённый каталог» и «разрешённый домен» субагенту доступны, а всё
// остальное — нет.
//
// Отказ вместо вопроса — единственный правильный ответ: диалог субагента
// человеку не показывают, спросить некого, а тихо пропустить значило бы
// сделать делегирование обходом всех проверок.
