...
// TestKimiTierFromModels — детекция тарифа по списку моделей.
...
// TestKimiWindowLabel — форматирование rolling-окна.
...
// TestKimiResetHint — форматирование ISO-времени в "обновится через X".
func TestKimiResetHint(t *testing.T) {
	...
	if got := kimiResetHint(past); got != "обновление" {
		t.Errorf("past ISO should give 'обновление', got %q", got)
	}
	// Через ~2ч должно содержать "обновится".
	future := time.Now().Add(2 * time.Hour).UTC().Format(time.RFC3339)
	got := kimiResetHint(future)
	if !strings.HasPrefix(got, "обновится") {
		t.Errorf("future ISO should start with 'обновится', got %q", got)
	}
}

// TestFetchKimiAvailableModels_OK — успешный ответ /models.
...
// TestFetchKimiAvailableModels_Empty — сервер вернул 401/500/пустое.
...
func TestFetchKimiUsage_FullPayload(t *testing.T) {
	...
	// Проверяем ключевые куски.
	// humanInt(1000) → "1.0k" (форматирование для читаемости), поэтому не "300 / 1000".
	// В rolling-окне используем Detail.Name если он есть ("5h limit"),
	// duration/timeUnit — fallback.
	mustContain := []string{
		"Kimi Code",
		"Тариф: K3 + HighSpeed",
		"Weekly limit",
		"30%",         // 300/1000
		"300 / 1.0k",  // humanInt форматирует
		"осталось 700",
		"5h limit",    // приоритет — Detail.Name из payload'а
		"40 / 100",
	}
...
// TestFetchKimiUsage_HTTPError — сервер отдал 500.
...
// TestFetchKimiUsage_StringNumbers — сервер шлёт used/limit как строки.
// Регрессия: prod вернул "cannot unmarshal string into Go struct field .usage.limit"
// потому что структура ждала int64.
func TestFetchKimiUsage_StringNumbers(t *testing.T) {
	future := time.Now().Add(3 * 24 * time.Hour).UTC().Format(time.RFC3339)
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		// Все числа — строки. Точно то что вернул api.kimi.com в проде.
		w.Write([]byte(`{
...
// TestParseFlexInt — универсальный парсер: число, число-в-строке, float, float-в-строке.
...
// TestFetchKimiUsage_BaseURLNormalization — base с и без /v1 суффикса.
