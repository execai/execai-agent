...
// TestDeriveKimiTier — детекция тарифа Kimi по списку моделей на подписке.
...
// TestSourceLabel_KimiWithTier — статус-бар показывает tier из AvailableModels.
func TestSourceLabel_KimiWithTier(t *testing.T) {
	...
	label := s.SourceLabel()
	// Ожидаем "kimi (K3)" — tier из моделей приоритетнее чем Plan="coding".
...
// TestSourceLabel_KimiFallbackToPlan — нет AvailableModels → показываем Plan.
...
// TestSourceLabel_ExecAI — дефолтный источник.
