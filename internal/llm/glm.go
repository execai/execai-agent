...
func (c *GLMClient) Stream(ctx context.Context, messages []AIMessage, tools []map[string]any, cb StreamCallbacks) (*StreamResult, error) {
	// OpenAI-совместимые провайдеры (OpenAI, OpenRouter, Moonshot, Z.ai) ждут
	// картинку как {"type":"image_url","image_url":{"url":"data:…"}}, а наш
	// внутренний блок держит строку. Без перевода мультимодальные модели через
	// эти источники картинок просто не видели.
	messages = withOpenAIImages(messages)
...
// ZAIAPIModels — network fallback for the Z.ai open platform (pay-per-token).
// Same models as the Coding Plan, but Provider is "zai-api": the two catalogs
// must stay distinguishable, a request with the wrong Provider goes to the
// wrong endpoint. The real list is fetched at /connect via GET /models.
func ZAIAPIModels() []Model {
	return []Model{
		{ID: "glm-5.2", Provider: "zai-api", Name: "GLM-5.2", Description: "Flagship coding (MoE 753B/40B, dual thinking)", Tier: "flagship", IsPrimary: true, HasTools: true},
		{ID: "glm-4.7", Provider: "zai-api", Name: "GLM-4.7", Description: "Быстрее и дешевле 5.2", Tier: "standard", HasTools: true},
	}
}

// withOpenAIImages переводит блоки картинок в форму OpenAI. Сообщения
// копируются: history принадлежит вызывающему и не должен меняться.
func withOpenAIImages(messages []AIMessage) []AIMessage {
	...
	for i, m := range out {
		...
		conv := make([]map[string]any, 0, len(blocks))
		for _, b := range blocks {
			if b.Type == "image_url" && b.ImageURL != "" {
			...
			conv = append(conv, map[string]any{"type": "text", "text": b.Text})
		}
		out[i].Content = conv
	}
	return out
