...
func SystemPrompt(cwd string, toolNames []string, memory string) string {
	...
	fmt.Fprintln(&b)
	fmt.Fprintln(&b, "### Если в своей памяти пусто")
	fmt.Fprintln(&b)
	fmt.Fprintln(&b, "На машине может жить память другого агента — у Claude Code это")
	claudeDir := "~/.claude/projects/<путь-проекта-через-дефисы>/memory/"
	fmt.Fprintf(&b, "`%s` (для этого проекта: `%s`), плюс `~/.claude/CLAUDE.md` и `CLAUDE.md` в репозитории.\n", claudeDir, ClaudeProjectMemoryDir(cwd))
	fmt.Fprintln(&b, "Если своя память молчит, а вопрос явно про накопленный контекст — загляни туда Grep'ом.")
	fmt.Fprintln(&b, "Нашёл полезное — НЕ копируй молча: скажи, что нашёл и у кого, и предложи `/memory find <запрос>`,")
	fmt.Fprintln(&b, "который покажет находки и спросит разрешение забрать их в свою память. Чужая память — не наша,")
	fmt.Fprintln(&b, "пока человек не разрешил.")
	fmt.Fprintln(&b)
