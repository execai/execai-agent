...
// SystemPrompt собирает финальный system prompt: базовая инструкция + список
// инструментов (краткое описание прикладывается отдельно через tools API LLM,
// здесь — только подсказки про модель работы) + project/user memory.
...
// memoryPaths — абсолютные пути для system prompt.
//   userDir   — папка с per-факт файлами
//   userIndex — индекс (MEMORY.md, всегда в контексте)
//   project   — project memory (single-file, в CWD)
