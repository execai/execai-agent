...
// Read-only префиксы. Это approximation: если команда после tримминга начинается с
// одного из этих токенов и не содержит rm/mv/cp/sed -i/>/>>/&&/||/;/$(/`,
// то считаем её безопасной и не спрашиваем пользователя.
...
// danger — паттерны, которые делают команду небезопасной даже если префикс в whitelist.
...
// Execute — обычный путь без стриминга. Вызывает ExecuteStream с no-op колбэком.
func (b *BashTool) Execute(ctx context.Context, args json.RawMessage) (string, error) {
	return b.ExecuteStream(ctx, args, func(string) {})
}

// ExecuteStream запускает команду и стримит stdout+stderr построчно через onChunk.
// Финальный return — весь накопленный вывод (с пометкой exit-code если ненулевой).
// Гарантия: onChunk вызывается в одной горутине (сериализовано).
func (b *BashTool) ExecuteStream(ctx context.Context, args json.RawMessage, onChunk func(string)) (string, error) {
	...
	// Собираем полный вывод в total, параллельно шлём строки в onChunk.
	// Мьютекс — onChunk сериализован, total пишется только из 2-х горутин.
	...
	streamReader := func(r io.Reader) {
		...
		// Большой буфер на случай длинных строк (по умолчанию 64KB).
