...
// ReadOnly is the toolset given to subagents: everything needed to investigate,
// nothing that changes the machine.
//
// Two things are absent on purpose. Write/Edit/Bash — a subagent's actions are
// never shown to the user for approval, so it must not be able to act. Task —
// a subagent spawning subagents multiplies the user's provider quota spend
// invisibly.
func ReadOnly(cwd string) *Registry {
	r := NewRegistry()
	r.Register(&ReadTool{cwd: cwd})
	r.Register(&GrepTool{cwd: cwd})
	r.Register(&GlobTool{cwd: cwd})
