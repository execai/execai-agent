...
// matchGlob — поддержка ** для рекурсии. Стандартный filepath.Match не понимает **.
func matchGlob(pat, path string) bool {
	if !strings.Contains(pat, "**") {
		...
		// Также проверяем по basename (типичный случай '*.go').
		ok2, _ := filepath.Match(pat, filepath.Base(path))
		return ok2
	}
	// Преобразуем ** в произвольный префикс из директорий.
	...
	for i, part := range parts {
		...
		// Ищем match part-сегмента после idx с учётом filepath.Match.
