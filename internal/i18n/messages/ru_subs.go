...
var ruSubsMessages = map[string]string{
	...
	"subs.provider.openrouter": "OpenRouter (sk-or-… с openrouter.ai — все вендоры одним ключом, pay-per-token)",
	...
	"subs.connectHint.openrouter": "OpenRouter pay-per-token: Claude, GPT, Gemini, DeepSeek одним ключом",
	...
	"subs.connect.example.openrouter": "Пример: /connect openrouter sk-or-v1-XXXXX  (ключ из https://openrouter.ai/keys)",
	...
	"subs.connect.zaiApiRejected": "✗ api.z.ai отклонил запрос (HTTP %d).\n" +
		"У Z.ai ОДИН ключ и на Coding Plan, и на открытую платформу,\n" +
		"поэтому дело почти наверняка не в «не том ключе», а в нулевом балансе\n" +
		"pay-per-token: подписка биллится отдельно от открытой платформы.\n" +
		"\n" +
		"Пополнить: https://z.ai/manage-apikey/apikey-list — либо остаться на\n" +
		"подписке через /connect zai (см. оговорку про её условия).",
	...
	"subs.connect.zaiKeyReused":       "переиспользован ключ подписки, он тот же самый",
	"subs.connect.openrouterRejected": "✗ ключ отвергнут openrouter.ai (HTTP %d).\n  Нужен ключ вида sk-or-… из https://openrouter.ai/keys.",
}
