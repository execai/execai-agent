...
Подсказка: Enter — отправить, Shift+Enter — перенос строки.`,
	...
	"hint.memoryFind":           "искать в своей памяти, а если пусто — у других агентов на машине",
	"memory.usage":              "/memory — статус · /memory import — забрать память других агентов отсюда · /memory export [каталог] — выгрузить память в markdown · /memory find <запрос> — искать в своей памяти, потом у других агентов",
	"memory.findUsage":          "Что искать? Пример: /memory find тарифы",
	"memory.findMine":           "🧠 Нашёл в своей памяти (%d) по запросу «%s»:",
	"memory.findForeign":        "📁 В своей памяти пусто, но нашлось у другого агента на этой машине (%d) по запросу «%s»:",
	"memory.findNothing":        "Ничего не нашлось ни в своей памяти, ни у других агентов: «%s»",
	"memory.findImportQuestion": "Забрать это к себе в память? (%d файл(ов))",
	...
	"project.serveQuestion":     "Запустить фоновый слушатель, чтобы принимать задачи из веб-чата?",
	"project.serveYes":          "Запустить",
	"project.serveYesDesc":      "инструменты — по твоему permissions.json; работает после закрытия терминала",
	"project.serveReadOnly":     "Только чтение",
	"project.serveReadOnlyDesc": "смотреть можно, менять файлы и запускать команды — нет",
	"project.serveNo":           "Не сейчас",
	"project.serveNoDesc":       "позже вручную: execai serve",
	"project.serveStarted":      "▶ Слушатель запущен (pid %d). Состояние: execai serve --status · остановить: --stop",
	"project.serveLog":          "  вывод: %s",
	"project.serveSkipped":      "Слушатель не запущен — задачи из веба будут ждать в очереди. Запустить: execai serve",
	"project.serveFailed":       "Не удалось запустить слушатель: %v",
	"project.serveAlready":      "▶ Слушатель уже работает (pid %d)",
