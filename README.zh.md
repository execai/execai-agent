...
- **名单之外的，代理会直接在网页聊天里问你** *（自 v6.18 起）*：任务进行中会弹出问题，显示代理想执行的确切命令，选项与 TUI 相同 —— 「一次」「本任务内整个工具」「本任务内这条命令」「永久允许」「拒绝」。「永久允许」会写入机器上的 `permissions.json`。约 2 分钟内未回答（或标签页已关闭）→ 代理收到拒绝：沉默永远不会扩大权限。
