...
execai — 终端优先的 AI 编程助手，与你喜欢的 LLM 提供方（Claude、GPT-5、Kimi K3、GLM-5.2 等）无缝协作 — 用你自己的订阅，或者用我们的。这是一个 Claude Code 风格的 CLI agent，真正跑在你自己的机器上：读文件、执行命令、与 kubernetes 对话，然后把结果聊给你听。它支持 **11 个来源** — 我们的 ExecAI 后端 + 你自己的 Z.ai / Kimi Code / Anthropic / OpenAI 订阅 + 本地的 Claude Code / OpenAI Codex CLI / Ollama（云端或本地）。你可以随时在它们之间切换，会话历史保持共享。
...
> **ℹ ExecAI 账号是可选的。** 默认源 `execai` 使用我们的后端（在 [execai.ru](https://execai.ru) 注册），但 CLI 完全可以**无需登录 ExecAI** 使用：直接在登录界面连接你自己的订阅 — `/connect kimi <key>`、`/connect zai <key>`、`/connect openai <key>`、`/connect anthropic <key>`，或本地 `claude-cli` / `codex-cli` / `ollama` — 然后 `/source <provider>` 开始工作。
...
10. [项目与后台模式 —— 网页 → 代理](#项目与后台模式--网页--代理)
11. [消费 / `/usage`](#消费--usage)
12. [命令与快捷键](#命令与快捷键)
13. [会话与历史](#会话与历史)
14. [文件位置](#文件位置)
15. [故障排查](#故障排查)
...
二进制文件默认来自 **GitHub Releases**。在俄罗斯/独联体地区 Yandex 镜像通常快得多 — 脚本会为 `ru` locale 自动选择，也可以用 `MIRROR=yandex` 强制指定。
...
如果你更愿意自己拿二进制文件而不运行安装脚本 — 所有版本连同校验和都在 GitHub Releases：

**https://github.com/execai/execai-agent/releases/latest**

选择你平台对应的归档文件（`execai-{linux,darwin,windows}-{amd64,arm64}.{tar.gz,zip}`），对照 `SHA256SUMS` 校验，解压后放到 PATH 中。
...
你可以在 `chat.execai.ru/settings/agents`（左下角 UserMenu → "Agents"，终端图标）查看所有 agent 并解绑任意一个。
...
| **LS** | 列出目录 |
| **Tree** | 按深度 N 显示文件树 |
| **WebFetch** | 打开网页：HTML → 可读文本 + 可继续访问的链接（不执行 JS） |
| **WebSearch** | 联网搜索，返回答案与引用来源（需要 ExecAI 账号） |
| **AskUser** | 当决定权在你时，提出带 2–4 个选项的问题 |
| **Task** | 把独立的调研任务交给只读子智能体 |
...
Agent 可以对接 **11 个不同的来源**。会话历史是共享的 — 随意切换，不会丢失上下文。

| 源 | 是什么 | 计费 |
|---|---|---|
| **execai**（默认） | 我们的网关 → 约 34 个模型任选 | ExecAI 套餐 |
| **zai** | 直连 Z.ai GLM Coding Plan | 你的 Z.ai 订阅 |
| **zai-api** | Z.ai 开放平台，按量计费 —— **与订阅同一把密钥**，且不受 Coding Plan 的工具清单限制 | Z.ai 按量计费 |
...
| **openrouter** | OpenRouter — 一个密钥调用所有厂商（Claude、GPT、Gemini、DeepSeek、Qwen…） | OpenRouter 按量计费 |
...
| **reference** | 指向外部系统的指针 | "Jenkins: jenkins.mycompany.com, Grafana dashboards" |
...
先在智能体自己的记忆中搜索。如果没有，就去看本机其他智能体的记忆 — Claude Code
的项目目录（`~/.claude/projects/<项目>/memory/`）、全局目录和 `CLAUDE.md` — 然后
**询问是否把找到的内容收进我们的记忆**。不会悄悄复制：找到和收下是两回事，后者由你决定。

---

## 项目与后台模式 —— 网页 → 代理

*自 v6.17 起。网页侧需要 execai.ru 项目目录中的「代理」工具 —— 正在逐步开放。*

你的机器可以成为**网页聊天项目里的一个工具**：把目录绑定到项目一次，启动后台监听器 —— 然后在浏览器里让模型在你的机器上干活。模型像调用其他项目工具（ssh、git）一样调用代理，任务**在项目目录里**执行，答案回到聊天中。

### 绑定目录

```
/project              # 你的项目；● —— 已绑定到当前目录
/project bind <名称>  # 绑定当前目录 + 把这台机器加入项目
/project on|off       # 与网页项目卡片里相同的开关
/project unbind       # 解除绑定并把机器从项目移除
```

绑定做两件事：记住「这台机器上的这个目录 = 那个项目」，并向项目添加一条普通的工具记录 —— 就像 ssh 配置一样 —— 机器会出现在网页项目卡片中，带有开/关开关。界面显示机器别名；稳定的机器 id 在重新登录后保持不变。
...
- 任务**在其项目绑定的目录中**执行，而不是 `serve` 启动的地方。目录不存在 → 明确报错，而不是在错误的地方运行。
- **允许的工具 = 你的 `permissions.json`**（你在 TUI 中用「永久允许」确认过的）。文件为空 → 全部允许，启动时会大声警告。
- **名单之外的，代理会直接在网页聊天里问你** *（自 v6.33 起）*：任务进行中会弹出问题，显示代理想执行的确切命令，选项与 TUI 相同 —— 「一次」「本任务内整个工具」「本任务内这条命令」「永久允许」「拒绝」。「永久允许」会写入机器上的 `permissions.json`。约 2 分钟内未回答（或标签页已关闭）→ 代理收到拒绝：沉默永远不会扩大权限。
- `--read-only` —— 只看不动模式：不改文件、不跑命令。
- 每次工具调用都写入**审计日志** `~/.config/execai/serve-audit.log`（8 MB 轮转）—— 可以看到代理夜里做了什么。
- 每台机器一个守护进程（pid-lock）。`execai serve --status` 显示 pid/运行时长/接入点；`--stop` 平滑停止（让当前任务做完）；`--stop --force` 5 秒后 SIGKILL —— 当前任务的结果会丢失，聊天里会看到超时。
- 代理离线 → 聊天在约 12 秒内诚实收到「代理无响应」；任务留在队列中，`serve` 一启动就会执行。投递需要确认（ack）—— 发进死连接的任务不会丢失。
- 关闭终端，进程随之结束。想让它活下去：
  `setsid nohup execai serve > ~/.execai-serve.log 2>&1 &`

此模式下 AskUser 被禁用（指模型的澄清提问；权限提问见上文，会进入网页聊天），迭代上限更低（每任务 30 次）。
...
| `/project [bind <名称>\|on\|off\|unbind]` | 把目录绑定到网页聊天项目；on/off —— 项目开关（自 v6.17） |
...
- Bug/功能请求：[github.com/execai/execai-agent/issues](https://github.com/execai/execai-agent/issues)
