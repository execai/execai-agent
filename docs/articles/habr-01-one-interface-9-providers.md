...
В какой-то момент выяснилось, что пользователям нужен не «агент с одной моделью», а мультитул: у кого-то подписка Kimi Code за $5, у кого-то Z.ai Coding Plan за $3, у кого-то корпоративный ключ Anthropic, а кто-то хочет гонять Ollama локально и не платить вообще. И всё это — в одном чате, с общей историей, с переключением на лету.
...
[СКРИН 1: /source picker с меню провайдеров]
...
[СКРИН 2: чат с Kimi K3 — виден reasoning приглушённым и ответ]
...
[СКРИН 3: /usage с прогресс-барами недельной квоты и 5h-окна]
...
**Kimi K3** (Moonshot AI, подписка Kimi Code от $5/мес) — флагман с нативным thinking mode и контекстом до 256K (до 1M на старших тарифах). В агентных задачах ведёт себя очень уверенно: сам решает, когда дёрнуть kubectl, аккуратно строит цепочки инструментов, reasoning читается осмысленно. Важная деталь для интеграции: **ID моделей строгие** — сервер принимает `k3`, `kimi-for-coding`, `kimi-for-coding-highspeed`, а привычное `kimi-k3` отдаёт 401, что при отладке выглядит как «ключ не подходит» и знатно путает.

**GLM-5.2** (Z.ai, Coding Plan от $3/мес) — MoE 753B/40B с dual thinking, заточенный под кодинг. Из необычного — суффикс-модификатор контекста прямо в ID модели: `glm-5.2[1m]` включает окно в 1M токенов. По нашему опыту GLM-5.2 — лучший «рефакторщик» в этой ценовой категории: длинные правки по многим файлам держит стабильно.
...
[СКРИН 4: /model — каталог моделей активного источника]

## Что в итоге

Ядро — интерфейс из пяти строк и четыре его реализации. Всё остальное — аккуратная сантехника: два диалекта SSE, склейка tool calls, динамические каталоги, инварианты переключения. Зато пользователь получает одну команду `/source` и свободу: Kimi K3 за $5, GLM-5.2 за $3, Claude по ключу, Ollama бесплатно локально — в одном чате с общей историей.
