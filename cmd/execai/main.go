...
// version подменяется при сборке через -ldflags "-X main.version=..."
var version = "dev"
...
func main() {
	...
	root := &cobra.Command{
		...
		SilenceUsage:  true,
		SilenceErrors: true,
		// Без аргументов открываем TUI-чат. Login/logout/config — slash-команды
		// внутри TUI; subcommands ниже остались для скриптов (CI, curl|bash).
...
func newChatCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// Если stdout не TTY — TUI бессмысленен (его нельзя нормально
			// отрендерить), валимся на plain без флага.
