// syscheck — e2e smoke-проверка всех 4 source. НЕ в основной build,
// запускается руками: `go run ./cmd/syscheck`. Шлёт реальные запросы
// (по 1 в каждый source). Использует текущий ~/.config/execai/.
//
// Цель — отловить регрессию вроде BUG-1 (возврат на ExecAI с чужим
// провайдером → 401) без TUI и без ручного клика.
package main
...
func run() error {
	...
	// 1. ExecAI — берём реальные model+provider из каталога биллинга,
	// иначе gateway отвергает пустые.
